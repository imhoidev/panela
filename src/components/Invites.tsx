  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (o) load(); }}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="sm" disabled={!canManage}><Link2 className="h-4 w-4 mr-1" />Convites</Button>
      </DialogTrigger>
      <DialogContent className="max-w-md">
        <DialogHeader><DialogTitle>Convites</DialogTitle></DialogHeader>
        <div className="space-y-3">
          {canManage && (
            <div className="flex gap-2 items-end">
              <div className="flex-1 space-y-1">
                <Label className="text-xs">Max usos</Label>
                <Input value={maxUses} onChange={(e) => setMaxUses(e.target.value)} placeholder="Ilimitado" className="h-8" />
              </div>
              <div className="flex-1 space-y-1">
                <Label className="text-xs">Expirar em (horas)</Label>
                <Input value={expiresH} onChange={(e) => setExpiresH(e.target.value)} placeholder="Nunca" className="h-8" />
              </div>
              <Button size="sm" onClick={create} className="h-8"><RefreshCw className="h-3 w-3 mr-1" />Criar</Button>
            </div>
          )}
          {invites.map((inv) => (
            <div key={inv.id} className="flex items-center justify-between rounded-lg border border-border px-3 py-2">
              ...
              <div className="flex gap-1">
                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => { navigator.clipboard.writeText(`${origin}/invite/${inv.code}`); toast.success("Link copiado!"); }}>
                  <Copy className="h-3.5 w-3.5" />
                </Button>
                <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => removeInvite(inv.id)}>

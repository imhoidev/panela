...
function Signup() {
  ...
  return (
    <Card className="w-full max-w-sm p-6 sm:p-8 space-y-6 border-border/60 bg-card/80 backdrop-blur-xl shadow-2xl shadow-black/40 rounded-2xl relative overflow-hidden">
      <div className="absolute -top-12 -left-12 h-32 w-32 bg-primary/10 rounded-full blur-2xl pointer-events-none" />

      <div className="flex flex-col items-center space-y-2 text-center">
        <PanelaLogo size={36} />
        <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Criar sua conta</h1>
        <p className="text-xs text-muted-foreground">Junte-se a milhares de membros na PANELA</p>
      </div>
      ...
      <form onSubmit={onEmail} className="space-y-4">
        <div className="space-y-1.5">
          <Label htmlFor="u" className="text-xs font-medium">Nome de usuário</Label>
          <Input id="u" required placeholder="seunome" value={username} onChange={(e) => setUsername(e.target.value)} className="h-10 text-sm bg-background/50" />
          <p className="text-[10px] text-muted-foreground">Será o seu identificador único na PANELA</p>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <Input id="pw" type={showPw ? "text" : "password"} required placeholder="Mínimo 8 caracteres" value={password} onChange={(e) => setPassword(e.target.value)} className="h-10 pr-10 text-sm bg-background/50" />
        ...
        <Button type="submit" className="w-full h-10 font-semibold text-sm shadow-md shadow-primary/20" disabled={loading}>
          {loading ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : "Criar conta gratuita"}
        </Button>
      </form>
